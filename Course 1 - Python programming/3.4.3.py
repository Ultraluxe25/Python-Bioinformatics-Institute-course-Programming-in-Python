...
with open('dataset_3363_4.txt', 'r') as file:
    n = 0  # count quantity of lines in file
    students_results = []
    avg_mat = avg_phy = avg_rus = 0
    for line in file:
        n += 1
        surname, math, physics, russian = line.split(";")
        students_results.append((int(math) + int(physics) + int(russian)) / 3)
        avg_mat += int(math)
        avg_phy += int(physics)
        avg_rus += int(russian)
    avg_mat /= n
...
with open('response_task3.txt', 'w') as response:
    for student in students_results:
        print(student)
    print(avg_mat, avg_phy, avg_rus)
